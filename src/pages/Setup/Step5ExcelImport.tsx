...
export default function Step5ExcelImport({ onNext, onBack }: Step5ExcelImportProps) {
  ...
  return (
    <div className="space-y-6 w-full max-w-5xl mx-auto">
      ...
      {/* ── Success banner ── */}
      {importedCount !== null && (
        <div className="bg-[#00d632]/10 border border-[#00d632]/30 rounded-xl px-4 py-3 text-center">
      ...
      {rows.length === 0 && !parsing && importedCount === null && (
        <div className="space-y-3">
          ...
          />
          {parseError && (
            <p className="text-[#ff4757] text-sm bg-[#ff4757]/10 rounded-lg px-3 py-2">{parseError}</p>
          )}
        </div>
      )}
            ...
            <table className="w-full text-sm" style={{ minWidth: 860 }}>
              <thead>
                <tr className="text-white/40 text-xs border-b border-white/10">
                  ...
                  <th className="py-2.5 pr-3 text-left" style={{ minWidth: 120 }}>Buy Date</th>
                ...
                {rows.map((row) => (
                  <React.Fragment key={row.rowKey}>
                    <tr className={`transition-opacity ${!row.selected ? 'opacity-35' : ''}`}>
                      ...
                      {/* P&L */}
                      <td className={`py-2.5 pr-3 text-right font-mono text-xs ${row.totalPL >= 0 ? 'text-[#00d632]' : 'text-[#ff4757]'}`}>
                        {row.totalPL >= 0 ? '+' : ''}{fmtNum(row.totalPL, row.currency)}
                      </td>

                      {/* Yield */}
                      <td className={`py-2.5 pr-3 text-right font-mono text-xs ${row.totalYield >= 0 ? 'text-[#00d632]' : 'text-[#ff4757]'}`}>
